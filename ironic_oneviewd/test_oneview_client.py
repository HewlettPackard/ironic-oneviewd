...
class OneViewRequestAPITestCase(unittest.TestCase):
    ...
    @mock.patch.object(OneViewRequestAPI, '_try_execute_request',
                       autospec=True)
    @mock.patch.object(OneViewRequestAPI, '_check_request_status',
                       autospec=True)
    ...
    def test__new_token_valid_login(
        self, mock_username, mock_password, mock_manage_url, mock_status,
        mock_requests):
        response = requests.Response()
        response.json = mock.MagicMock(return_value={'sessionID': 'logged'})
        mock_requests.return_value = response
    ...
    @mock.patch.object(OneViewRequestAPI, '_check_request_status',
                       autospec=True)
    @mock.patch.object(oneview_conf, 'get_manager_url', autospec=True)
    def test_prepare_and_do_request(
        self, mock_manage_url, mock_status, mock_requests):
        response = requests.Response()
        response.json = mock.MagicMock(return_value={'sessionID': 'logged'})
        mock_requests.return_value = response
        mock_status.return_value = False
...
class OneViewServerProfileAPITestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.ov_server_profile_api = OneViewServerProfileAPI()

    @mock.patch.object(OneViewRequestAPI, 'prepare_and_do_request', autospec=True)
    def test_list_empty(self, mock_request):
        mock_request.return_value = {'members': []}
        self.assertEquals(len(self.ov_server_profile_api.list()), 0)

    @mock.patch.object(OneViewRequestAPI, 'prepare_and_do_request', autospec=True)
    def test_list(self, mock_request):
        mock_request.return_value = {'members': [{'serverHardwareUri': 'sh1_uri'},
                                                 {'serverHardwareUri': 'sh2_uri'},
                                                 {'serverHardwareUri': 'sh3_uri'}]}
        self.assertEquals(len(self.ov_server_profile_api.list()), 3)

    @mock.patch.object(OneViewRequestAPI, 'prepare_and_do_request', autospec=True)
    def test_server_profile_template_list_empty(self, mock_request):
        mock_request.return_value = {'members': []}
        self.assertEquals(len(self.ov_server_profile_api.list()), 0)
        self.assertEquals(len(self.ov_server_profile_api.server_profile_template_list()), 0)

    @mock.patch.object(OneViewRequestAPI, 'prepare_and_do_request', autospec=True)
    def test_server_profile_template_list(self, mock_request):
        mock_request.return_value = {'members': [{'serverHardwareUri': None},
                                                 {'serverHardwareUri': 'sh2_uri'},
                                                 {'serverHardwareUri': None}]}
        self.assertEquals(len(self.ov_server_profile_api.list()), 3)
        self.assertEquals(len(self.ov_server_profile_api.server_profile_template_list()), 2)
